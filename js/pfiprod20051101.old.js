function createProductRenderer(shop, prodInfo, dimensioner, sizer, looks) {
    return {
        ...
        getBreadCrumb: function() {
            var levels = [{title: 'Shop', url: '/shop.html'}, {title: 'Happy Everyday', url: this.getCategoryURL()}, {title: this.product.name}];
        ...
        createInfoDiv: function (varIdx, szIdx) {
            var res = '<h4 class="mb-2">' + this.product.name + '</h4>'
...
function createPageComponent(prodInfo, dimensioner, sizer, looks, catalog) {
    return {
        ...
        onSelectionChange: function() {
            ...
            var imageHTML = '<div class="col-12 col-md-7">' + renderer.createImageDiv(valIdx) + '</div>';
            $(prodImgSelector).append(imageHTML);

            this.updateImageCarousel();

            var szIdx = renderer.getSizeIdx(valSize);
            var infoHTML = '<div class="col-12 col-md-5 pl-lg-10">' + renderer.createInfoDiv(valIdx, szIdx) + '</div>';
